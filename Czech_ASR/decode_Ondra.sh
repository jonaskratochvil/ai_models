...
dict_src=$data/Ondra/local/dict
lm_src=$data/Ondra/local/lang/

lang=$data/Ondra/lang
dict=$data/Ondra/dict
dict_tmp=$data/Ondra/dict_tmp
graph=$data/Ondra/graph
...
# Celkovy postup
# v 00/lm_texts run cleanme.sh na novy text - to ho vycisti pro LM. Pak v lexicon_experiments updatuj novy lexicon tim textem, pomoci local/prepare_lm.sh natrenuj novy LM. Pak do nove slozky v datech nakopiruj novy LM a lexikon a zkompiluj graf znovu
...
#gzip  $lm_src.gz
#utils/format_lm.sh $dict $lm_src/lm_interpolate.gz $dict_src/lexicon.txt $lang
...
steps/online/nnet3/prepare_online_decoding.sh --mfcc-config conf/mfcc_hires.conf $dict exp/nnet3/extractor $data/exp/chain/tdnn1a_sp $data/Ondra
fi
...
if [ $stage -le 20 ]; then
     ...
     online2-wav-nnet3-latgen-faster \
       ...
       --config=$data/Ondra/conf/online.conf \
